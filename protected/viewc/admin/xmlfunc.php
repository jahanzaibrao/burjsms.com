...
function count_smsc_status($smscs)
{
    ...
    if ($smscs['count'] > 1) {
        foreach ($smscs['smsc'] as $smsc) {
            //var_dump($smsc['status']);
            foreach ($stats as $st => $i) {
                if (substr($smsc['status'], 0, strlen($st)) == $st) {
                    isset($stats[$st]) ? $stats[$st]++ : $stats[$st] = 1;
                }
            }
        }
    } else {
        foreach ($stats as $st => $i) {
            if (substr($smscs['smsc']['status'], 0, strlen($st)) == $st) {
                isset($stats[$st]) ? $stats[$st]++ : $stats[$st] = 1;
            }
        }
    }
        ...
        switch ($mode) {
            case 'red':
                /* return "<a href=\"#\" class=href onClick=\"do_alert('".
                       ...
                       "');\"><span class=\"label label-important\"><b>".
                       $smsc_status[$state].
                       "</b> links</span></a>";*/
                return "<span class=\"label label-danger\"><b>" .
                    $smsc_status[$state] .
                    "</b> links</span></a>";
                break;
            case 'green':
                return "<span class=\"label label-success\"><b>" .
                    $smsc_status[$state] .
                    "</b> links</span></a>";
                break;
        }
 ...
 */
function admin_link($mode, $config)
{
    //global $config;
