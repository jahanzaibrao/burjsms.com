                ...
                <div class="col-md-12">
                    <div class="widget p-lg">
                        <div class="row no-gutter">
                            <h3 class="page-title-sc"><?php echo SCTEXT('Edit SMS Template') ?><small><?php echo SCTEXT('modify sms template content and other options') ?></small></h3>
                            <hr>
                            ...
                            <div class="col-md-12">
                                ...
                                <form class="form-horizontal" method="post" id="tmp_form" action="">
                                    <input type="hidden" name="tempid" value="<?php echo $data['temp']->id ?>" />
                                    <div class="form-group">
                                        <label class="control-label col-md-3"><?php echo SCTEXT('Template Name') ?>:</label>
                                        <div class="col-md-8">
                                            <input type="text" name="tname" id="tname" class="form-control" placeholder="<?php echo SCTEXT('enter name for your sms template') ?>. . . ." maxlength="100" value="<?php echo $data['temp']->title ?>" />
                                        </div>
                                    </div>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="col-md-8">
                                            <div class="checkbox checkbox-success">
                                                <input id="toggle-rt" name="tglrt" <?php if ($data['temp']->route_id != '0') { ?> checked <?php } ?> class="toggle" type="checkbox">
                                                <label for="toggle-rt"><?php echo SCTEXT('Request Approval for Template Based Route') ?></label>
                                            </div>
                                            <?php if ($_SESSION['user']['account_type'] == '0') { ?>
                                                <div id="rtbox" class="m-t-md <?php if ($data['temp']->route_id == '0') { ?> hidden <?php } ?> wd100">
                                                    <select class="form-control wd100" data-plugin="select2" name="route">
                                                        <?php foreach ($_SESSION['credits']['routes'] as $rt) { ?>
                                                            <option <?php if ($data['temp']->route_id == $rt['id']) { ?> selected <?php } ?> value="<?php echo $rt['id'] ?>"><?php echo $rt['name'] ?></option>
                                                        <?php } ?>
                                                    </select>
                                                </div>
                                            <?php } else { ?>
