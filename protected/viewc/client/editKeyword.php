                    ...
                    <div class="col-md-12">
                        <div class="widget p-lg">
                            <div class="row no-gutter">
                                <h3 class="page-title-sc"><?php echo SCTEXT('Edit Keyword')?><small><?php echo SCTEXT('edit the parameters of the primary keyword')?></small></h3>
                                <hr>
                                <?php include('notification.php') ?>
                                <div class="col-md-12">
                                <!-- start content -->
                                    <form class="form-horizontal" action="" method="post" id="kwfrm">
                                    <input type="hidden" name="kid" value="<?php echo $data['keyword']->id ?>">
                                        <div class="form-group">
                                            <label class="control-label col-md-3"><?php echo SCTEXT('Enter Keyword')?>:</label>
                                            <div class="col-md-8">
                                                <input readonly value="<?php echo $data['keyword']->keyword ?>" type="text" class="form-control" name="keyword" id="keyword" placeholder="<?php echo SCTEXT('enter your keyword')?>">
                                            </div>
                                        </div>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <div class="col-md-8">
                                                <?php if(sizeof($data['vmns']) > 0 && $data['keyword']->added_by==$_SESSION['user']['userid']){ ?>
                                                <select class="form-control" data-plugin="select2" name="vmn">
                                                    <?php foreach($data['vmns'] as $vmn){ ?>
                                                    <option <?php if($data['keyword']->vmn==$vmn->id){ ?> selected <?php } ?> value="<?php echo $vmn->id ?>"><?php echo $vmn->vmn ?></option>
                                                    <?php } ?>
                                                </select>
                                                    <?php } else { ?>
                                                        <input type="text" class="form-control" readonly value="<?php echo $data['kw_vmn'] ?>">
                                                        <input type="hidden" name="vmn" value="<?php echo $data['keyword']->vmn ?>">
                                                    <?php } ?>
                                            </div>
                                        </div>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <div class="col-md-8">
                                                <input value="<?php echo $data['keyword']->forward_sms_to==0?'':$data['keyword']->forward_sms_to; ?>" type="text" maxlength="15" class="form-control" name="fwdmob" id="fwdmob" placeholder="<?php echo SCTEXT('enter mobile number with country code')?>">
