                ...
                <div class="col-md-12">
                    <div class="widget p-lg">
                        <div class="row no-gutter">
                            <h3 class="page-title-sc"><?php echo SCTEXT('Edit Sender ID') ?><small><?php echo SCTEXT('modify your sender ID') ?></small></h3>
                            <hr>
                            ...
                            <div class="col-md-12">
                                ...
                                <form class="form-horizontal" method="post" id="sid_form" action="">
                                    <input type="hidden" name="senid" value="<?php echo $data['sender']->id ?>" />
                                    <div class="form-group">
                                        <label class="control-label col-md-3"><?php echo SCTEXT('Sender ID') ?>:</label>
                                        <div class="col-md-8">
                                            <input type="text" name="sid" id="sid" class="form-control" placeholder="<?php echo SCTEXT('enter sender ID') ?>..." value="<?php echo $data['sender']->sender_id ?>" maxlength="100" />
                                    ...
                                    <?php
                                    $countries_matrix = $data['sender']->countries_matrix != "" ? json_decode($data['sender']->countries_matrix, true) : ["countries" => [], "operators" => []];
                                    if ($countries_matrix['countries'][0] != 0) {
                                        $multiple_cv = 1;
                                    }
                                    if ($countries_matrix['operators'][0] != 0) {
                                        $multiple_op = 1;
                                    }
                                    ?>
                                    <div class="form-group">
                                        ...
                                        <div class="col-md-8">
                                            <div class="clearfix">
                                                <div class="col-md-6 p-r-sm">
                                                    <select class="form-control" <?php if ($multiple_cv == 1) { ?> multiple <?php } ?> data-plugin="select2" id="cvsel" name="cvsel[]" data-options="{ templateSelection: function (data){ if(!data.element){return ''};  var nstr = '<div class=\'text-center\'>'+data.text+'</div>';return $(nstr); } }">
                                                        <option value="0"> <?php echo SCTEXT('All Countries') ?> </option>
                                                        <?php foreach ($data['cvdata'] as $cv) { ?>
                                                            <option <?php if (in_array($cv->prefix, $countries_matrix['countries'])) { ?> selected <?php } ?> value="<?php echo $cv->prefix ?>"> <?php echo $cv->country . ' (+' . $cv->prefix . ')' ?></option>
                                                        <?php } ?>
                                                    </select>
                                                </div>
                                                <div class="col-md-6 p-r-sm">
                                                    <select class="form-control" data-plugin="select2" <?php if ($multiple_op == 1) { ?> multiple <?php } ?> id="opsel" name="opsel[]" data-options="{ templateSelection: function (data){ if(!data.element){return ''};  var nstr = '<div class=\'text-center\'>'+data.text+'</div>';return $(nstr); } }">
                                                        <option value="0"> <?php echo SCTEXT('All Operators') ?> </option>
                                                        <?php foreach ($data['opdata'] as $op) { ?>
                                                            <option class="opitem" <?php if (in_array(base64_encode($op->brand . '|' . $op->country_iso), $countries_matrix['operators'])) { ?> selected <?php } ?> data-cpre="<?php echo $op->country_code ?>" value="<?php echo base64_encode($op->brand . '|' . $op->country_iso) ?>"> <?php echo $op->brand . ' (' . $op->country_iso . ')' ?></option>
                                                        <?php } ?>
                                                    </select>
                                                </div>
                                            </div>
                                    ...
                                    <?php $tfiles = explode(",", $data['sender']->file_ids);  ?>
                                    <div class="form-group">
                                        ...
                                        <div class="col-md-8">
                                            <select multiple class="form-control" name="tfiles[]" data-plugin="select2" data-options="{placeholder: '<?php echo SCTEXT('Choose Files') ?> ...'}">
                                                <?php foreach ($data['docs'] as $doc) { ?>
                                                    <option value="<?php echo $doc->id ?>" <?php if (is_array($tfiles) && in_array($doc->id, $tfiles)) { ?> selected <?php } ?>> <?php echo $doc->filename ?></option>
                                                <?php } ?>
                                            </select>
