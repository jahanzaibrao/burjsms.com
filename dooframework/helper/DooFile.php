...
class DooFile
{
    ...
    public function upload($uploadPath, $filename, $rename = '')
    {
        ...
        if (is_array($file['name']) === False) {
            $ext = strtolower($this->getFileExtensionFromPath($file['name']));
            ...
            foreach ($file['error'] as $k => $error) {
                ...
                if ($error == UPLOAD_ERR_OK) {
                    $ext = strtolower($this->getFileExtensionFromPath($file['name'][$k]));
     ...
     */
    public function readFileContents($fullFilePath, $flags = 0, mixed $context = null, $offset = -1, $maxlen = null)
    {
