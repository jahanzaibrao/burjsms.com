...
class DooWebApp
{
    ...
    public function run()
    {
        ...
        try {
            ...
            $error_id = DooSmppcubeHelper::getToken(7);
            $timestamp = strtotime(date('Y-m-d H:i:s'));
            $error = print_r($th, true);
            $filename = '/var/www/html/systemErrors/' . $timestamp . '-ERROR-' . $error_id . '.txt';

            $myfile = fopen($filename, "w");
            fwrite($myfile, $error);
            fclose($myfile);
            header("location:" . Doo::conf()->APP_URL . 'systemError/' . $error_id);
        }
    }
