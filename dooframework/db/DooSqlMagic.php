...
class DooSqlMagic
{
    ...
    public function connect()
    {
        ...
        $i = 1;
        foreach ($this->dbconfig as $dbconf) {
            try {
                if ($dbconf[4] == 'sqlite')
                    $this->{'pdo' . $i} = new PDO("{$dbconf[4]}:{$dbconf[0]}");
                else
                    $this->{'pdo' . $i} = new PDO("{$dbconf[4]}:host={$dbconf[0]};dbname={$dbconf[1]}", $dbconf[2], $dbconf[3], array(PDO::ATTR_PERSISTENT => $dbconf[5]));
                $this->{'pdo' . $i}->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
                $this->connected = true;
                if (isset($dbconf['charset']) && isset($dbconf['collate'])) {
                    $this->{'pdo' . $i}->exec("SET NAMES '" . $dbconf['charset'] . "' COLLATE '" . $dbconf['collate'] . "'");
                } else if (isset($dbconf['charset'])) {
                    $this->{'pdo' . $i}->exec("SET NAMES '" . $dbconf['charset'] . "'");
                }
                $this->{'pdo' . $i}->exec("SET NAMES 'utf8mb4'");
            } catch (PDOException $e) {
                throw new SqlMagicException('Failed to open the DB connection No ' . $i, SqlMagicException::DBConnectionError);
            }
            $i++;
        }
    }
     ...
     */
    public function beginTransaction($dbno = 1)
    {
        if ($this->transactionLevel === 0) {
            $this->{'pdo' . $dbno}->beginTransaction();
        } else {
            $this->{'pdo' . $dbno}->exec("SAVEPOINT LEVEL{$this->transactionLevel}");
        }
     ...
     */
    public function commit($dbno = 1)
    {
        $this->transactionLevel--;
        if ($this->transactionLevel === 0) {
            $this->{'pdo' . $dbno}->commit();
        } else {
            $this->{'pdo' . $dbno}->exec("RELEASE SAVEPOINT LEVEL{$this->transactionLevel}");
        }
    }
     ...
     */
    public function rollBack($dbno = 1)
    {
        $this->transactionLevel--;
        if ($this->transactionLevel === 0) {
            $this->{'pdo' . $dbno}->rollBack();
        } else {
            $this->{'pdo' . $dbno}->exec("ROLLBACK TO SAVEPOINT LEVEL{$this->transactionLevel}");
        }
    }
     ...
     * @return PDOStatement
     */
    public function query($query, $param = null, $dbno = 1, $return_fetch = 0)
    {
        ...
        $stmt = $this->{'pdo' . $dbno}->prepare($query);
    ...
    */
    public function fetchAll($query, $param = null, $fetchMode = null, $fetchModeOptionalParam1 = null, $fetchModeOptionalParam2 = array(), $dbno = 1)
    {
        $stmt = $this->query($query, $param, $dbno);
        if ($fetchMode !== null) {
            switch ($fetchMode) {
                case PDO::FETCH_COLUMN:
                    $stmt->setFetchMode($fetchMode, ($fetchModeOptionalParam1 !== null) ? $fetchModeOptionalParam1 : 0);
     ...
     */
    public function quote($string, $type = null, $dbno = 1)
    {
        return $this->{'pdo' . $dbno}->quote($string, $type);
    }
     ...
     * @return int
     */
    public function lastInsertId($dbno = 1)
    {
        return $this->{'pdo' . $dbno}->lastInsertId();
    }
     ...
     * @return PDO
     */
    public function getDbObject($dbno = 1)
    {
        return $this->{'pdo' . $dbno};
    }
     ...
     * @return mixed A model object or associateve array of the queried result
     */
    public function find($model, $opt = null, $dbno = 1)
    {
        if (is_object($model)) {
        ...
        if (isset($opt['select'])) {
            ...
            $sqladd['select'] = implode(', ', $model->_fields);
        }
        ...
        if (isset($opt['param']) && isset($where_values))
            $rs = $this->query($sql, array_merge($opt['param'], $where_values), $dbno);
        else if (isset($opt['param']))
            $rs = $this->query($sql, $opt['param'], $dbno);
        else if (isset($where_values))
            $rs = $this->query($sql, $where_values, $dbno);
        else
            $rs = $this->query($sql, NULL, $dbno);
     ...
     */
    public function insert($model, $dbno = 1)
    {
        $class_name = get_class($model);
        ...
        foreach ($obj as $o => $v) {
            if (isset($v) && in_array($o, $model->_fields)) {
                ...
                if ($this->dbconfig[$dbno - 1][4] == 'mysql') {
                    $fieldstr .= '`' . $o . '`,';
                } else {
                    $fieldstr .= $o . ',';
                }
     ...
     * @return int The inserted record's Id
     */
    public function insertAttributes($model, $data, $dbno = 1)
    {
        if (is_string($model)) {
     ...
     */
    public function update($model, $opt = NULL, $dbno = 1)
    {
        //add values to fields where the model propertie(s) are/is set
        $obj = get_object_vars($model);
        ...
        //echo $sql;
        return $this->query($sql, $values, $dbno)->rowCount();
    }
     ...
     * @return int Number of rows affected
     */
    public function update_attributes($model, $data, $opt = NULL, $dbno = 1)
    {
        if (is_string($model)) {
            $model = $this->loadModel($model, true);
            $table = $model->_table;
     ...
     */
    public function relatedUpdate($model, $rmodels, $opt = NULL, $dbno = 1)
    {
        $this->update($model, $opt, $dbno);
        foreach ($rmodels as $rmodel) {
            ...
            //insert the related model with the relation's foreign key
                ...
                if ($rtype == 'has_many' && isset($rparams['foreign_key']) && isset($rparams['through'])) {
                    ...
                    //check if the related model already exist it true than insert to the 'through' table with the 2 ids.
                    $chk_rmodel = $this->find($rmodel, array('select' => $fieldstr, 'limit' => 1), $dbno);
                    if ($chk_rmodel != NULL) {
                        //echo '<h1>'.$chk_rmodel->{$rparams['foreign_key']}.'</h1>';
                        $rId = $chk_rmodel->{$chk_rmodel->_primarykey};
                    } else {
                        //echo '<h2>Not found this related model in many-to-many. Insert it!</h2>';
                        $rId = $this->insert($rmodel, $dbno);
                    }

                    //insert into the 'through' Table, if exists dun create duplicates
                    if ($this->query("SELECT {$rparams['foreign_key']},{$model_linked_key} FROM {$rparams['through']} WHERE {$rparams['foreign_key']}=? AND {$model_linked_key}=?", array($rId, $mId), $dbno)->fetch() == NULL) {
                        $this->query("INSERT INTO {$rparams['through']} ({$model_linked_key},{$rparams['foreign_key']}) VALUES (?,?)", array($mId, $rId), $dbno);
                    }
                } else if (isset($rparams['foreign_key'])) {
                    //echo '<h2>Insert 1to1 or 1tomany</h2>';
                    list($rtype, $rparams) = self::relationType($this->map, $class_name, $rclass_name);
                    $rmodel->{$rparams['foreign_key']} = $model->{$model->_primarykey};
                    $this->insert($rmodel, $dbno);
                }
            }
        }
    }
     ...
     */
    public function deleteAll($model, $dbno = 1)
    {
        if (!is_object($model)) {
            $this->loadModel($model);
            $model = new $model;
    ...
    //Author: saurav
    public function truncateTable($model, $dbno = 1)
    {
        if (!is_object($model)) {
            $this->loadModel($model);
            $model = new $model;
        ...
        $sql = "TRUNCATE TABLE {$model->_table}";
        $rs = $this->query($sql, NULL, $dbno);
    }
    public function optimizeTable($model, $dbno = 1)
    {
        if (!is_object($model)) {
            $this->loadModel($model);
            $model = new $model;
        ...
        $sql = "OPTIMIZE TABLE {$model->_table}";
        $rs = $this->query($sql, NULL, $dbno);
    }
     ...
     */
    public function delete($model, $opt = NULL, $dbno = 1)
    {
        if (is_object($model)) {
            //add values to fields where the model propertie(s) are/is set
            $obj = get_object_vars($model);
