...
if( isset($_POST['name']) && empty($_POST['spam-check']) ) {
	...
	$input_subject = strip_tags($_POST['subject'])==''?'Contact Request':strip_tags($_POST['subject']);
	...
	$message .= "\n---\nThis email was sent by SMS Panel contact form.";

	// Now check to see if there are any errors 
	if( !$error ) {

		// No errors, send mail using conditional to ensure it was sent
		if( mail(base64_decode($_POST['cemail']), $subject, $message, "From: $input_email") ) {
