...
class JSLikeHTMLElement extends DOMElement
{
	...
	*/
	public function __set($name, $value) {
		if ($name == 'innerHTML') {
			...
			trigger_error('Undefined property via __set(): '.esc_html($name).' in '.esc_html($trace[0]['file']).' on line '.esc_html($trace[0]['line']), E_USER_NOTICE);
		}
	}
	...
	*/	
	public function __get($name)
	{
		...
		$trace = debug_backtrace();
		trigger_error('Undefined property via __get(): '.esc_html($name).' in '.esc_html($trace[0]['file']).' on line '.esc_html($trace[0]['line']), E_USER_NOTICE);
		return null;
	}
